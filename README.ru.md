...
| `apps/products/tests/test_products.py` | Список/карточка/создание, денормализация рейтинга, кэш карточки, фасеты поиска и автокомплит, рекомендации и fallback, email продавца не раскрыт, эндпоинт размерной сетки и маппинг категория-группа, Q&A вопросы/ответы/лайк «полезно» (права, сортировка по полезности, бейдж продавца), ответ продавца на отзывы и агрегация обратной связи (владение 403, роль-гейт, фильтр/сортировка по ответу, ответ виден на карточке) |
| `apps/orders/tests/test_orders.py` | Создание заказа, списание stock, валидация, отмена покупателем с возвратом, авторизация статуса в мультивендоре, оформление выбранного подмножества, снимок варианта, список/детали заказов продавца (владение, фильтр статуса, read-only смешанного, PII покупателя не утекает) |
