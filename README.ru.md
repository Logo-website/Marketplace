...
Проект развёрнут вживую (учебный / портфельный, pre-launch):

- **Витрина:** https://marketplace-web-z98x.onrender.com
- **API / Swagger:** https://marketplace-api-rusx.onrender.com/api/docs/

**На прод вынесено только ядро — тяжёлый стек сознательно оставлен за бортом.** Хостинг: Render (Web Service для API, Static Site для фронта, Key Value для Redis) + Neon (managed PostgreSQL) + Resend (почта). Elasticsearch, RabbitMQ/Celery, Kafka + `node_service`, ClickHouse и C++-рекомендатель **не** развёрнуты: им нужен managed-хостинг или выделенная RAM — это дорого и неоправданно для учебного pre-launch проекта без реальной нагрузки. Код написан с мягкими фолбэками, поэтому отсутствие сервиса деградирует одну фичу, а не роняет приложение.

| Возможность | На проде | Если выключено — видимый эффект |
|---|---|---|
| Каталог, карточки, корзина, чекаут, заказы | Да | — |
| Регистрация / вход / смена email (OTP через Resend) | Да | — |
| Письма о заказах и аккаунте (Resend, синхронно) | Да | — |
| Отзывы, рейтинги, вопросы, бренды, образы, чат, админка, модерация | Да | — |
| Полнотекстовый поиск (Elasticsearch) | Нет | `/search` отдаёт 503; каталог при этом работает |
| Живые уведомления (Kafka + WebSocket) | Нет | лента-колокольчик хранится в БД, просто не в реальном времени |
| Маркетинговые рассылки (Celery) | Нет | админ-рассылка не отправляется |
| Аналитика продавца (ClickHouse) | Нет | графики в дашборде пустые |
| Рекомендации (C++ ко-покупки) | Нет | фолбэк на «популярное по категории» |

Про free-тариф: API засыпает после ~15 мин простоя (первый запрос после этого может грузиться до ~50 с); PostgreSQL на free-тарифе Neon (без удаления по времени, в отличие от Render).
