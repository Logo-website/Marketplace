...
### Заказы
- Модели: `Order`, `OrderItem` (снимок `product_name`, `price_at_purchase`, `size`, `color`).
- Создание с позициями: проверка статуса, остатков, атомарное списание.
- Оформление из корзины: `POST /api/orders/from-cart/` (опциональный `items` оформляет только выбранные позиции, остальное остаётся в корзине).
...
| `apps/products/tests/test_products.py` | Список/карточка/создание, денормализация рейтинга, кэш карточки, фасеты поиска и автокомплит, рекомендации и fallback, email продавца не раскрыт, эндпоинт размерной сетки и маппинг категория-группа, Q&A вопросы/ответы/лайк «полезно» (права, сортировка по полезности, бейдж продавца) |
| `apps/orders/tests/test_orders.py` | Создание заказа, списание stock, валидация, отмена покупателем с возвратом, авторизация статуса в мультивендоре, оформление выбранного подмножества, снимок варианта |
