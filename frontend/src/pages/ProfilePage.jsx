...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState(
    searchParams.get('tab') === 'profile' ? 'profile' : 'main'
  )
  const phoneRequired = searchParams.get('phone') === 'required'
                ...
                <motion.div key="profile" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  ...
                  {phoneRequired && !user?.phone && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-xl text-sm flex items-center gap-2 mb-2"
                      >
