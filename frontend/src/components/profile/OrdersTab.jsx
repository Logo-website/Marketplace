...
const STATUS_CONFIG = {
  created:    { label: 'Создан',       color: 'bg-surface text-ink-soft',        icon: '🕐' },
  paid:       { label: 'Оплачен',      color: 'bg-accent-soft text-accent',      icon: '💳' },
  processing: { label: 'В обработке',  color: 'bg-warning/10 text-warning',      icon: '⚙️' },
  shipped:    { label: 'Отправлен',    color: 'bg-accent-soft text-accent',      icon: '🚚' },
  ...
  return (
    <motion.div key="orders" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-xl font-black text-ink">Мои заказы</h2>
              ...
              <motion.div key={order.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="bg-card rounded-2xl overflow-hidden border border-line">
                <button onClick={() => setExpanded(isExpanded ? null : order.id)} className="w-full p-5 flex items-center justify-between hover:bg-surface transition gap-3">
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="w-10 h-10 bg-surface rounded-xl flex items-center justify-center text-lg border border-line shrink-0">{st.icon}</div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className="font-black text-ink">{Number(order.total_price).toLocaleString()} ₽</span>
                  ...
                  {isExpanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-line overflow-hidden">
                      <div className="p-5 flex flex-col gap-2">
                        ...
                        {order.recipient_name && <p className="text-sm text-ink-faint">👤 {order.recipient_name}{order.recipient_phone ? `, ${order.recipient_phone}` : ''}</p>}
                        {order.delivery_address && <p className="text-sm text-ink-faint">📍 {DELIVERY_LABELS[order.delivery_method] ?? 'Доставка'}: {order.delivery_address}</p>}
                        {order.payment_method && <p className="text-sm text-ink-faint">💳 {PAYMENT_LABELS[order.payment_method] ?? 'Оплата'}</p>}
                        <div className="mt-1">
