...
export default function OrderDetail({ order, busy, onStatus, onCancel }) {
  ...
  return (
    <div className="p-5 border-t border-line flex flex-col gap-3">
      {/* Данные покупателя для исполнения */}
      <div className="flex flex-col gap-1.5 text-sm">
        <p className="text-ink-soft"><span className="text-ink-faint">Получатель:</span> {order.buyer_name || '—'}</p>
        <p className="text-ink-soft break-words"><span className="text-ink-faint">Адрес:</span> {order.delivery_address || '—'}</p>
        {order.comment && (
          <p className="text-ink-soft break-words"><span className="text-ink-faint">Комментарий:</span> {order.comment}</p>
        )}
      </div>
      ...
      <div className="mt-1">
        {(order.items || []).map((item) => (
          <div key={item.id} className="flex justify-between text-sm py-2 border-b border-line last:border-0 gap-4">
        ...
        <div className="flex justify-between font-bold text-ink text-sm mt-2">
      ...
      <div className="flex flex-wrap items-center gap-2 mt-2">
        {order.can_update_status ? (
          ...
          <span className="text-xs font-medium text-warning bg-warning/10 px-3 py-2 rounded-xl">
            Совместный заказ - статус ведёт площадка
          </span>
        )}
        <button
          onClick={() => printShippingLabel(order)}
          className="text-xs font-semibold text-ink-soft border border-line-strong px-4 py-2 rounded-xl hover:bg-surface transition"
        >
          Печать этикетки
        </button>
      </div>
